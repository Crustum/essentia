...
it('outputs json when ESSENTIA_FORCE is set without an agent', function (): void {
    $output = decodeOutput(runWith('pest', 'PassingTest', withAgent: false, extraEnv: ['ESSENTIA_FORCE' => '1']));

    expect($output['result'])->toBe('passed')
        ->and($output['tests'])->toBe(2);
});

it('outputs normal pest output when ESSENTIA_FORCE is falsy without an agent', function (string $value): void {
    $process = runWith('pest', 'PassingTest', withAgent: false, extraEnv: ['ESSENTIA_FORCE' => $value]);
