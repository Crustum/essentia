...
it('compresses cake help command alignment spacing', function (): void {
    $line = '  bootstrap                     BootstrapUI commands entry point.';

    expect(OutputCleaner::clean($line))->toBe(' bootstrap BootstrapUI commands entry point.');
});

it('compresses indented saloon list spacing', function (): void {
    expect(OutputCleaner::clean('  Connectors: 16'))->toBe(' Connectors: 16');
});
